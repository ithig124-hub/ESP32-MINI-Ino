...
void serviceClock() {
  ...
  if (now - lastSecond >= 1000) {
    ...
    if (rtcAvailable && rtc.isRunning()) {
      ...
      // FIX: Use getter methods instead of private members
...
void setup() {
  ...
  if (qmi.begin(Wire, 0x6B, IIC_SDA, IIC_SCL)) {  // QMI8658 I2C address
    // FIX: Correct parameter order for configAccelerometer
    // Signature: configAccelerometer(AccelRange range, AccelODR odr, LpfMode lpfOdr)
